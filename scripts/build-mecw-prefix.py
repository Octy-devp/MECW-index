...
def build():
    ...
    lines.append("")
    lines.append("### DCA EXTRACTION FROM THIS TEXT (Gotha):")
    lines.append("```yaml")
    lines.append("reasoning: |")
    lines.append("  The 1875 Gotha merger program replaced scientific socialism with")
    lines.append("  Lassallean slogans. Marx diagnoses this as principled retreat —")
    lines.append("  theoretical gains of Capital traded for organizational unity. The")
    lines.append("  lag is triple: Lassalle didn't understand wages (theory), merger")
    lines.append("  pressure forced concessions (organization), and a 'servile belief")
    lines.append("  in the state' persisted (ideology). Marx corrects each error and")
    lines.append("  grounds the alternative in the material constraint that Phase 1")
    lines.append("  communism 'still bears the birthmarks of the old society.'")
    lines.append("")
    ...
    lines.append("```yaml")
    lines.append("reasoning: |")
    lines.append("  The Manifesto traces how the bourgeoisie emerged from feudal")
    lines.append("  society to create its antagonist. The 1848 revolutionary")
    lines.append("  conjuncture is the crisis. The lag is dual: organizational")
    lines.append("  (workers compete) and ideological (bourgeois ideas mask")
    lines.append("  exploitation). The alternative is a revolutionary political")
    lines.append("  program, constrained by national differences. Complete DCA.")
    lines.append("")
    lines.append("crisis_diagnosis:")
    ...
    lines.append("0. CHAIN-OF-THOUGHT: Before the structured YAML, include a `reasoning` field")
    lines.append("   that explains your analytical process in prose. Cite specific textual")
    lines.append("   evidence. This makes your extraction auditable by humans — the reasoning")
    lines.append("   must be sufficient for a domain expert to verify or challenge your DCA.")
